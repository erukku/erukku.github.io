import { Container, Graphics, Text, TextStyle } from "./newpixi.js"
import {CanvasTextMetrics,Ticker}  from "pixi.js"
...
export default class TextInput extends Container{
	constructor(styles){
        ...
        this._addListeners()
        this.worldVisible = ((this.globalDisplayStatus & 0b10) !== 0)
        //console.log(this.worldVisible)
	}
	...
	_createDOMInput(){
        ...
        //this._dom_input.title = "a";
		
		for(let key in this._input_style){
            console.log(key,this._input_style[key])
	...
	_updateDOMInput(){
        console.log(this._canvas_bounds)
	...
	_updateSurrogateHitbox(bounds){
		...
		this._surrogate_hitbox.fill(0x000000)
	...
	_updateSurrogateMask(bounds,padding){
		...
		this._surrogate_mask.fill(0x000000)
	...
	_updateFontMetrics(){
        ...
        function toFontString(style) {
            ...
            const fontVariant = style.fontVariant || "nomal";
            return `${fontStyle} ${fontVariant} ${fontWeight} ${fontSize}px "${fontFamily}"`;
        }
        //const font = style.toFontString()
        const font = toFontString(style);
        console.log(font);
        var TextMetrics = CanvasTextMetrics;
	...
	_getCanvasBounds(){
        ...
        let rect = this._last_renderer.view.canvas.getBoundingClientRect();
        if(rect.top == NaN){
            rect.top = 0;
        }
        if(rect.left == NaN){
	...
	_getDOMInputBounds(){
        ...
        //let bounds = this._dom_input.view;
	...
	_getDOMRelativeWorldTransform(){
        //console.log(this._last_renderer.view.canvas);
		let canvas_bounds = this._last_renderer.view.canvas.getBoundingClientRect()
